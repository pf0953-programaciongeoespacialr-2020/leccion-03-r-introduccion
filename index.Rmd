...
* Visualización de datos: [From data to Viz | Find the graphic you need](https://www.data-to-viz.com/)

## Descripción general del lenguaje R

[R](https://www.r-project.org/) es un lenguaje de programación enfocado en [análisis estadístico](https://en.wikipedia.org/wiki/Statistics). Es ampliamente utilizado en diversas áreas de investigación, entre las que pueden mencionarse [aprendizaje automático (_machine learning_)](https://en.wikipedia.org/wiki/Machine_learning), [ciencia de datos](https://en.wikipedia.org/wiki/Data_science) y [_big data_](https://en.wikipedia.org/wiki/Big_data), con aplicaciones en campos como biomedicina, bioinformática y finanzas, entre muchos otros. Fue creado por Ross Ihaka y Robert Gentleman en la Universidad de Auckland, Nueva Zelanda, en 1993.
...
Los parámetros de las funciones tienen nombres que pueden especificarse en caso de ser necesario. En el siguiente ejemplo, se utilizan los nombres de los parámetros ```xlab``` y ```ylab``` de la función ```plot()``` para especificar las etiquetas de los ejes x e y en un gráfico del conjunto de datos ```cars```.

```{r}
# Gráfico con etiquetas en los ejes x e y
plot(x=cars, xlab="Velocidad", ylab="Distancia")
```
...
ca_poblacion_paises = data.frame(pais = paises, poblacion = poblaciones)
...
## Graficación
Una de las principales fortalezas de R, es su capacidad para generar gráficos estadísticos. A continuación, se detallan algunas de las principales funciones.
...
**Ejemplos: población de países centroamericanos**  
...
```{r}
barplot(height=ca_poblacion_paises.poblacion$poblacion, names.arg=ca_poblacion_paises.poblacion$pais, xlab="País", ylab="Millones de habitantes", main="Población de los países centroamericanos")
```

Gráfico con barras horizontales
```{r}
barplot(height=ca_poblacion_paises.poblacion$poblacion, names.arg=ca_poblacion_paises.poblacion$pais, ylab="País", xlab="Millones de habitantes", main="Población de los países centroamericanos", horiz=TRUE)
```
